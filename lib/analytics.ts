...
const ENDPOINT = "https://e.iq-mermaid.com/e";
...
// footer) don't need the page threaded through props.
let currentPage = "Home";
// Rendered locale of the page the visitor is on. Stamped per event rather than
// per visit: one visit can cross locales (a /es visitor opening the English
// editor), and a per-visit value would describe only the first page.
...
// visitor clicks through quickly, and enough headroom under the endpoint's
// burst limit.
const FLUSH_MS = 2000;
...
interface QueuedEvent {
   ...
   *  beaconed batch must not collapse onto its delivery time. */
  ts: number;
...
// on the visitor's device and the pipeline stays consentless. Dies with the
// page; a fresh load simply starts token-less and the server falls back to
// the hash.
