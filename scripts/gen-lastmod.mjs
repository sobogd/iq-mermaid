...
const TRACKED = ["content", "app/_landing/legal-content.ts"];

/** A content file -> the sitemap key of the page it drives. */
function keyFor(file) {
  ...
  if (editor) return `editor:${editor[1]}`;
  const blog = file.match(/^content\/blog\/([a-z0-9-]+)\/([a-z]{2})\.json$/);
  if (blog) return `blog:${blog[2]}/${blog[1]}`;
  if (file === "content/blog/manifest.json") return "blog:index";
